...
export default class Auth extends Component {
    ...
    actionAuth = async (event) => {
        event.preventDefault();
        if (this.state.login && this.state.password) {
            await fetch(ApiUrl.auth, {
                method: 'POST',
                body: JSON.stringify({action: 'auth', login: this.state.login, password: this.state.password})
            }).then((res) => {
        ...
        return (
            <div>
                {!!!getCookie('account').token ? (
                    <Form onSubmit={this.actionAuth} className="form-auth">
                        ...
                        <Button variant="success" size="lg" type="submit" disabled={!(this.state.login && this.state.password && !this.state.message)} block>Войти</Button>
                    </Form>
                ) : (<Redirect to="/"/>)}
            </div>
        );
